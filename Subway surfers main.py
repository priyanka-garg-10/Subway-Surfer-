import cv2 #module -computer vision
import mediapipe as mp #module give us ready to use ml soln for cv
import pyautogui #module to simulate mouse control using gestures
mp_drawing = mp.solutions.drawing_utils #to draw landmarks and conectors
mp_holistic = mp.solutions.holistic #pose and face landmarks 
mp_hands = mp.solutions.hands #for tarcking fingers and hand
cap = cv2.VideoCapture(0) #0 means we r using laptop camera 
tipIds = [4, 8, 12, 16, 20] #4-eye,8-cheek,12-shoulder ,16-wrist,20-fingers 
game_started = 1 
charac_pos = [0,1,0]
index_pos = 1
fixedx = None
fixedy = None
rec = None
with mp_hands.Hands(min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands: #threshold value for detection
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic: #same as above 
        while True:
            success, frame = cap.read()
            frame = cv2.flip(frame, 1) 
            frame = cv2.resize(frame, (440,330)) # frame size 
            height, width, channel = frame.shape
            img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) #use to convert color 
